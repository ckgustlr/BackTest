...
def simulated_trade(data, short_max_counter=3, long_max_counter=3):
    ...
    for i, (timestamp, row) in enumerate(data.iterrows()):
        ...
        exit_long = False
        exit_short = False
        # 롱 진입: 직전 롱 진입가보다 0.1 하락 시
        if not long_positions or (last_long_entry is not None and current_price <= last_long_entry - 0.1):
            ...
            entry_long = True
        # 숏 진입: 직전 숏 진입가보다 0.1 상승 시
        if not short_positions or (last_short_entry is not None and current_price >= last_short_entry + 0.1):
