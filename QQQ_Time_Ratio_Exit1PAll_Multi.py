...
if __name__ == "__main__":
    try:
        ...
        if SINGLE_TEST_MODE:
            ...
            print("="*80)
            best_short_counter, best_long_counter = 20, 20
            best_short_denom, best_long_denom = 160, 160
        else:
            # 1~20 x 1~20 시간, 비율 1/50~1/110 (10단위 7개) 각각 → 20*20*7*7 = 19600 케이스, total_rate>100 드랍
            # CPU 멀티프로세싱으로 병렬 실행
            print("\n" + "="*80)
            print("Testing: SHORT_TIME=1~20, LONG_TIME=1~20, SHORT_RATIO=1/100~1/300(20), LONG_RATIO=1/100~1/300(20)")
            print("Total: 20*20*11*11 = 4840 cases (total_rate>100 드랍,SHORT_PROFIT<-10% 드랍, LONG_PROFIT<-10% 드랍)")
            n_workers = min(16, os.cpu_count() or 8)
            print(f"Parallel: ProcessPoolExecutor max_workers={n_workers}")
            print("="*80)
            
            short_counter_range = list(range(1, 20,1))   # 1~20
            long_counter_range = list(range(1, 20,1))    # 1~20
            ratio_denominators = list(range(100, 320, 20))  # 100,120,140,160,180,200,220,240,260,280,300 → 11개
